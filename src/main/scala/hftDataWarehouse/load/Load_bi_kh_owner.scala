...
object Load_bi_kh_owner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    CreateHFTHiveTableInfo.create_table_bi_kh_owner(spark,hivedatabase,isDropTable)

    //同步表相关信息定义
    val real_tableName = "bi_kh_owner"
    val dataFrame_temp_view = "bi_kh_owner_view"
    val customize_tableName = SelectRelectionDBTableUtils.select_bi_kh_owner()

    val task_name = "Load_bi_kh_owner"
