...
/**
 * 同步broker_approve_backup表
 */
object Load_broker_approve_backup {
  ...
  def main(args: Array[String]): Unit = {
    ...
    CreateHFTHiveTableInfo.create_table_broker_approve_backup(spark,hivedatabase,isDropTable)

    //同步表相关信息定义
    val real_tableName = "broker_approve_backup"
    val dataFrame_temp_view = "broker_approve_backup_view"
    val customize_tableName = SelectRelectionDBTableUtils.select_broker_approve_backup()

    val task_name = "Load_broker_approve_backup"
