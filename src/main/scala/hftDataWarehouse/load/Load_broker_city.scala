...
object Load_broker_city {
  ...
  def main(args: Array[String]): Unit = {
    ...
    CreateHFTHiveTableInfo.create_table_broker_city(spark,hivedatabase,isDropTable)

    //同步表相关信息定义
    val real_tableName = "broker_city"
    val dataFrame_temp_view = "broker_city_view"
    val customize_tableName = SelectRelectionDBTableUtils.select_broker_city()

    val task_name = "Load_broker_city"
