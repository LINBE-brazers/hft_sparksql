...
/**
 * 同步customer_online_order_pay_info
 */
object Load_customer_online_order_pay_info {
  ...
  def main(args: Array[String]): Unit = {
    ...
    CreateHFTHiveTableInfo.create_table_customer_online_order_pay_info(spark,hivedatabase,isDropTable)

    //同步表相关信息定义
    val real_tableName = "customer_online_order_pay_info"
    val dataFrame_temp_view = "customer_online_order_pay_info_view"
    val customize_tableName = SelectRelectionDBTableUtils.select_customer_online_order_pay_info()

    val task_name = "Load_customer_online_order_pay_info"
